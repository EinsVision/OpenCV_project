...
{
	// 15.1 kNN 알고리즘을 이용한 2차원 점 분류
	img2 = Mat::zeros(Size(500, 500), CV_8UC3);
	knn = KNearest::create();
...
void trainAndDisplay()
{
	...
	for (int i = 0; i < img2.rows; ++i) {
		for (int j = 0; j < img2.cols; ++j) {
			...
			if (response == 0)
				img2.at<Vec3b>(i, j) = Vec3b(128, 128, 255); // R
			else if (response == 1)
				img2.at<Vec3b>(i, j) = Vec3b(128, 255, 128); // G
			else if (response == 2)
				img2.at<Vec3b>(i, j) = Vec3b(255, 128, 128); // B
		}
	}
	...
	for (int i = 0; i < train.rows; i++)
	{
		...
		if (l == 0)
			circle(img2, Point(x, y), 5, Scalar(0, 0, 128), -1, LINE_AA);
		else if (l == 1)
			circle(img2, Point(x, y), 5, Scalar(0, 128, 0), -1, LINE_AA);
		else if (l == 2)
			circle(img2, Point(x, y), 5, Scalar(128, 0, 0), -1, LINE_AA);
	}
